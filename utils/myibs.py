...
class parallel_IBS:
   ...
   num_samples_per_call: int, optional
    ...
    def compute_logl(self, params, num_reps, weights, return_positive, start_time):
    ...
    def loop_ibs_sampling(self, params, simulated_data0, t0, num_reps):
        """
        A function to perform loop-based Inverse Biased Sampling (IBS) for the given parameters.
        ...
        for i_rep in range(num_reps):
            ...
            # if the algorithm reach the maximum runtime, break the loop
            if exit_flag==2:
                break 

            # if max runtime reached, set exit flag to 2
            if np.isfinite(self.max_time) and time.time() - t0 > self.max_time:
                exit_flag = 2
                try:
            ...
            # offset is the number of samples to be added to the hit trials
            offset = 1
            hits = np.zeros(n_trials, dtype=bool)
    ...
    def calc_logl_from_K(self, psi_table, K_matrix):
        """
        Convert matrix of K values into log-likelihoods.
